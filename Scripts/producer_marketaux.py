...
def poll():
    url = f"https://api.marketaux.com/v1/news/all?symbols=GOOGL,MSFT,AMZN,NVDA&api_token={os.getenv('MARKETAUX_API_KEY')}"
    try:
        res = requests.get(url).json()
        validated = MarketAuxResponse(**res)
        for article in validated.data:
            producer.produce(topic='raw_marketaux_news', key=article.uuid, value=article.model_dump_json())
        producer.flush()
