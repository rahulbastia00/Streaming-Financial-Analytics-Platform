...
def poll():
    for symbol in ["GOOGL", "MSFT", "AMZN", "NVDA"]:
        ...
        try:
            ...
            producer.produce(topic='raw_alphavantage_candles', key=symbol, value=validated.model_dump_json())
            producer.flush()
            print(f"[AlphaVantage] Ingested candles for {symbol}")
            time.sleep(15) # Wait 15 seconds between stocks so you don't break Alpha Vantage's free limit (5 calls per minute)
