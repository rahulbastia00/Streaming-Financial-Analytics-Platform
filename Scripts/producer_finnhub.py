    ...
    if raw_json.get("type") == "trade":
        try:
            ...
            for tick in validated.data:
                producer.produce(topic='raw_finnhub_ticks', key=tick.s, value=tick.model_dump_json())
            producer.flush()
...
ws = websocket.WebSocketApp(f"wss://ws.finnhub.io?token={os.getenv('FINNHUB_API_KEY')}", on_open=on_open, on_message=on_message)
ws.run_forever()